
const admin = require("firebase-admin");

const privateKey = "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n";

const serviceAccount = {
  projectId: "revallo-gaming-7642a",
  clientEmail: "firebase-adminsdk-fbsvc@revallo-gaming-7642a.iam.gserviceaccount.com",
  privateKey: privateKey.replace(/\\n/g, "\n")
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  });
}

const db = admin.firestore();

async function searchByNick() {
  const nicknames = ["Zeus_Apollo", "Zeus_Apollo3"];
  console.log(`Starting search for nicknames: ${nicknames.join(", ")}`);

  console.log("--- REGULAR TOURNAMENTS ---");
  const tSnap = await db.collection("tournaments").get();
  tSnap.forEach(doc => {
    const d = doc.data();
    if (nicknames.includes(d.nickname)) {
        console.log(`MATCH found in regular tournament ${doc.id}: ${d.title} | Organizer: ${d.organizer_id}`);
    }
  });

  console.log("\n--- MINI TOURNAMENTS ---");
  const mSnap = await db.collection("mini_tournaments").get();
  mSnap.forEach(doc => {
    const d = doc.data();
    if (nicknames.includes(d.nickname)) {
        console.log(`MATCH found in mini tournament ${doc.id}: ${d.title} | Organizer: ${d.organizer_id}`);
    }
  });
}

searchByNick().catch(console.error);
