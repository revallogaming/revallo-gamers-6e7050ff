
const admin = require("firebase-admin");

const privateKey = "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n";

const serviceAccount = {
  projectId: "revallo-gaming-7642a",
  clientEmail: "firebase-adminsdk-fbsvc@revallo-gaming-7642a.iam.gserviceaccount.com",
  privateKey: privateKey.replace(/\\n/g, "\n")
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  });
}

const db = admin.firestore();

async function manualScan() {
  const targetNicknames = ["Zeus_Apollo", "Zeus_Apollo3"];
  console.log(`Manually scanning all tournaments for nicknames: ${targetNicknames.join(", ")}`);

  console.log("--- SCANNING REGULAR TOURNAMENTS ---");
  const tSnapshot = await db.collection("tournaments").get();
  tSnapshot.forEach(doc => {
    const data = doc.data();
    const dataString = JSON.stringify(data);
    let found = false;
    for (const nick of targetNicknames) {
        if (dataString.includes(nick)) {
            found = true;
            break;
        }
    }
    if (found) {
        console.log(`Tournament MATCH: ${doc.id} | Title: ${data.title} | organizer_id: ${data.organizer_id} | data: ${dataString}`);
    }
  });

  console.log("\n--- SCANNING MINI TOURNAMENTS ---");
  const mSnapshot = await db.collection("mini_tournaments").get();
  mSnapshot.forEach(doc => {
    const data = doc.data();
    const dataString = JSON.stringify(data);
    let found = false;
    for (const nick of targetNicknames) {
        if (dataString.includes(nick)) {
            found = true;
            break;
        }
    }
    if (found) {
        console.log(`Mini Tournament MATCH: ${doc.id} | Title: ${data.title} | organizer_id: ${data.organizer_id} | data: ${dataString}`);
    }
  });
}

manualScan().catch(console.error);
