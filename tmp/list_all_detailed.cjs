...
async function listAllDetailed() {
  ...
  tSnap.forEach(doc => {
    const d = doc.data();
    console.log(`ID: ${doc.id} | Title: ${d.title} | Organizer: ${d.organizer_id} | CreatedAt: ${d.created_at ? (d.created_at.toDate ? d.created_at.toDate().toISOString() : d.created_at) : 'MISSING'}`);
  });
}
