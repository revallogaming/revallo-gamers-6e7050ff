
const admin = require("firebase-admin");

const serviceAccount = {
  projectId: "revallo-gaming-7642a",
  clientEmail: "firebase-adminsdk-fbsvc@revallo-gaming-7642a.iam.gserviceaccount.com",
  privateKey: "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n"
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  });
}

const db = admin.firestore();

async function migrateTournaments() {
  console.log("Starting migration for regular tournaments...");
  const tSnapshot = await db.collection("tournaments").get();
  let migratedCount = 0;

  for (const doc of tSnapshot.docs) {
    const data = doc.data();
    if (!data.organizer_id && data.creatorId) {
      console.log(`Migrating tournament ${doc.id}: setting organizer_id = ${data.creatorId}`);
      await doc.ref.update({
        organizer_id: data.creatorId,
        updated_at: admin.firestore.FieldValue.serverTimestamp()
      });
      migratedCount++;
    }
  }
  console.log(`Migrated ${migratedCount} regular tournaments.`);

  console.log("\nChecking mini tournaments...");
  const mSnapshot = await db.collection("mini_tournaments").get();
  migratedCount = 0;
  for (const doc of mSnapshot.docs) {
    const data = doc.data();
    if (!data.organizer_id && data.creator_id) {
       console.log(`Migrating mini tournament ${doc.id}: setting organizer_id = ${data.creator_id}`);
       await doc.ref.update({
         organizer_id: data.creator_id,
         updated_at: admin.firestore.FieldValue.serverTimestamp()
       });
       migratedCount++;
    }
  }
  console.log(`Migrated ${migratedCount} mini tournaments.`);
}

migrateTournaments().catch(console.error);
