
const admin = require("firebase-admin");

const privateKey = "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n";

const serviceAccount = {
  projectId: "revallo-gaming-7642a",
  clientEmail: "firebase-adminsdk-fbsvc@revallo-gaming-7642a.iam.gserviceaccount.com",
  privateKey: privateKey.replace(/\\n/g, "\n")
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  });
}

const db = admin.firestore();

async function migrateToActiveUser() {
  const oldUserId = "VgETyxpXqNeBYabcg0KB"; // Zeus_Apollo3
  const activeUserId = "tjYoxEwnOAf1WmCJpWG0"; // Zeus_Apollo
  
  console.log(`Starting final migration from ${oldUserId} to ${activeUserId}...`);

  // Migrate regular tournaments
  const tSnapshot = await db.collection("tournaments").where("organizer_id", "==", oldUserId).get();
  console.log(`Found ${tSnapshot.size} tournaments to migrate.`);
  for (const doc of tSnapshot.docs) {
    console.log(`Migrating tournament ${doc.id}`);
    await doc.ref.update({
      organizer_id: activeUserId,
      updated_at: admin.firestore.FieldValue.serverTimestamp()
    });
  }

  // Migrate mini tournaments (just in case)
  const mSnapshot = await db.collection("mini_tournaments").where("organizer_id", "==", oldUserId).get();
  console.log(`Found ${mSnapshot.size} mini tournaments to migrate.`);
  for (const doc of mSnapshot.docs) {
    console.log(`Migrating mini tournament ${doc.id}`);
    await doc.ref.update({
      organizer_id: activeUserId,
      updated_at: admin.firestore.FieldValue.serverTimestamp()
    });
  }

  console.log("Migration completed.");
}

migrateToActiveUser().catch(console.error);
