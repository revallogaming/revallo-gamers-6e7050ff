...
async function fullProfileDump() {
  console.log("=== FULL PROFILE DUMP ===");
  const pSnap = await db.collection("profiles").get();
  pSnap.forEach(doc => {
    console.log(`Profile ID: ${doc.id} | Data: ${JSON.stringify(doc.data())}`);
  });
}

fullProfileDump().catch(console.error);
