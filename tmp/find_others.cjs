
const admin = require("firebase-admin");

const privateKey = "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n";

const serviceAccount = {
  projectId: "revallo-gaming-7642a",
  clientEmail: "firebase-adminsdk-fbsvc@revallo-gaming-7642a.iam.gserviceaccount.com",
  privateKey: privateKey.replace(/\\n/g, "\n")
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  });
}

const db = admin.firestore();

async function findOthers() {
  const activeUserId = "tjYoxEwnOAf1WmCJpWG0";
  const tSnap = await db.collection("tournaments").get();
  console.log(`--- TOURNAMENTS NOT OWNED BY ${activeUserId} ---`);
  tSnap.forEach(doc => {
    const d = doc.data();
    if (d.organizer_id !== activeUserId && d.creatorId !== activeUserId) {
        console.log(`ID: ${doc.id} | Title: ${d.title} | Organizer: ${d.organizer_id} | creatorId: ${d.creatorId}`);
    }
  });

  const mSnap = await db.collection("mini_tournaments").get();
  console.log(`\n--- MINI TOURNAMENTS NOT OWNED BY ${activeUserId} ---`);
  mSnap.forEach(doc => {
    const d = doc.data();
    if (d.organizer_id !== activeUserId && d.creator_id !== activeUserId) {
        console.log(`ID: ${doc.id} | Title: ${d.title} | Organizer: ${d.organizer_id} | creator_id: ${d.creator_id}`);
    }
  });
}

findOthers().catch(console.error);
