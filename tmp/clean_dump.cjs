
const admin = require("firebase-admin");
const fs = require("fs");

const privateKey = "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n";

const serviceAccount = {
  projectId: "revallo-gaming-7642a",
  clientEmail: "firebase-adminsdk-fbsvc@revallo-gaming-7642a.iam.gserviceaccount.com",
  privateKey: privateKey.replace(/\\n/g, "\n")
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  });
}

const db = admin.firestore();

async function cleanDump() {
  let output = "=== CLEAN TOURNAMENT DUMP ===\n";
  const tSnapshot = await db.collection("tournaments").get();
  tSnapshot.forEach(doc => {
    output += `Tournament ID: ${doc.id} | Data: ${JSON.stringify(doc.data())}\n`;
  });

  output += "\n=== CLEAN MINI TOURNAMENT DUMP ===\n";
  const mSnapshot = await db.collection("mini_tournaments").get();
  mSnapshot.forEach(doc => {
    output += `Mini Tournament ID: ${doc.id} | Data: ${JSON.stringify(doc.data())}\n`;
  });

  fs.writeFileSync("d:\\Documentos\\revallo-gamers-6e7050ff\\tmp\\clean_dump.txt", output, "utf8");
  console.log("Clean dump written to tmp/clean_dump.txt");
}

cleanDump().catch(console.error);
