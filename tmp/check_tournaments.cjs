
const admin = require("firebase-admin");

const serviceAccount = {
  projectId: "revallo-gaming-7642a",
  clientEmail: "firebase-adminsdk-fbsvc@revallo-gaming-7642a.iam.gserviceaccount.com",
  privateKey: "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n"
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  });
}

const db = admin.firestore();

async function checkTournaments() {
  console.log("Checking regular tournaments...");
  const tSnapshot = await db.collection("tournaments").orderBy("created_at", "desc").limit(10).get();
  tSnapshot.forEach(doc => {
    const data = doc.data();
    console.log(`Tournament ID: ${doc.id}`);
    console.log(`Title: ${data.title}`);
    console.log(`organizer_id: ${data.organizer_id}`);
    console.log(`creatorId: ${data.creatorId}`);
    console.log("---");
  });

  console.log("\nChecking mini tournaments...");
  const mSnapshot = await db.collection("mini_tournaments").orderBy("created_at", "desc").limit(10).get();
  mSnapshot.forEach(doc => {
    const data = doc.data();
    console.log(`Mini Tournament ID: ${doc.id}`);
    console.log(`Title: ${data.title}`);
    console.log(`organizer_id: ${data.organizer_id}`);
    console.log("---");
  });
}

checkTournaments().catch(console.error);
