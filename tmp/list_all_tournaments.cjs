
const admin = require("firebase-admin");

const privateKey = "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n";

const serviceAccount = {
  projectId: "revallo-gaming-7642a",
  clientEmail: "firebase-adminsdk-fbsvc@revallo-gaming-7642a.iam.gserviceaccount.com",
  privateKey: privateKey.replace(/\\n/g, "\n")
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  });
}

const db = admin.firestore();

async function listAllTournaments() {
  const shortUID = "tjYoxEwnOAf1WmCJpWG0";
  const longUID = "tjYoxEwnOAf1WmCJpWG0rqj4dqJ3";
  const legacyUID = "VgETyxpXqNeBYabcg0KB";

  console.log("=== SCANNING TOURNAMENTS ===");
  const tSnap = await db.collection("tournaments").get();
  tSnap.forEach(doc => {
    const d = doc.data();
    const matchesUser = [d.organizer_id, d.creatorId, d.creator_id].some(id => 
        id === shortUID || id === longUID || id === legacyUID
    );
    if (matchesUser) {
        console.log(`MATCH: ${doc.id} | Title: ${d.title} | Org: ${d.organizer_id} | CrId: ${d.creatorId}`);
    }
  });

  console.log("\n=== SCANNING MINI TOURNAMENTS ===");
  const mSnap = await db.collection("mini_tournaments").get();
  mSnap.forEach(doc => {
    const d = doc.data();
    const matchesUser = [d.organizer_id, d.creatorId, d.creator_id].some(id => 
        id === shortUID || id === longUID || id === legacyUID
    );
    if (matchesUser) {
        console.log(`MATCH: ${doc.id} | Title: ${d.title} | Org: ${d.organizer_id} | CrId: ${d.creator_id}`);
    }
  });
}

listAllTournaments().catch(console.error);
