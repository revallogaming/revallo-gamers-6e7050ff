...
async function fetchAll() {
  ...
  tSnap.forEach(doc => {
    const d = doc.data();
    console.log(`Title: ${d.title} | Organizer: ${d.organizer_id} | creatorId: ${d.creatorId} | ID: ${doc.id}`);
  });
  ...
  mSnap.forEach(doc => {
    const d = doc.data();
    console.log(`Title: ${d.title} | Organizer: ${d.organizer_id} | creator_id: ${d.creator_id} | ID: ${doc.id}`);
  });
}
