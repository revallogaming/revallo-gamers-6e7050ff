
const admin = require("firebase-admin");

const serviceAccount = {
  projectId: "revallo-gaming-7642a",
  clientEmail: "firebase-adminsdk-fbsvc@revallo-gaming-7642a.iam.gserviceaccount.com",
  privateKey: "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n"
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  });
}

const db = admin.firestore();

async function countIssues() {
  const tSnapshot = await db.collection("tournaments").get();
  let count = 0;
  tSnapshot.forEach(doc => {
    const data = doc.data();
    if (!data.organizer_id) {
       count++;
       console.log(`Tournament ${doc.id} missing organizer_id. creatorId: ${data.creatorId}`);
    }
  });
  console.log(`Total regular tournaments missing organizer_id: ${count}`);

  const mSnapshot = await db.collection("mini_tournaments").get();
  count = 0;
  mSnapshot.forEach(doc => {
    const data = doc.data();
    if (!data.organizer_id) {
       count++;
       console.log(`Mini Tournament ${doc.id} missing organizer_id. creator_id: ${data.creator_id}`);
    }
  });
  console.log(`Total mini tournaments missing organizer_id: ${count}`);
}

countIssues().catch(console.error);
