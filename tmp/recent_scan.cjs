
const admin = require("firebase-admin");

const privateKey = "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n";

const serviceAccount = {
  projectId: "revallo-gaming-7642a",
  clientEmail: "firebase-adminsdk-fbsvc@revallo-gaming-7642a.iam.gserviceaccount.com",
  privateKey: privateKey.replace(/\\n/g, "\n")
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  });
}

const db = admin.firestore();

async function recentScan() {
  console.log("=== RECENT TOURNAMENTS (Last 50) ===");
  const tSnapshot = await db.collection("tournaments")
    .orderBy("created_at", "desc")
    .limit(50)
    .get();
  
  tSnapshot.forEach(doc => {
    const data = doc.data();
    console.log(`Tournament: ${doc.id} | Title: ${data.title} | Organizer: ${data.organizer_id} | Nickname: ${data.nickname || 'N/A'}`);
  });

  console.log("\n=== RECENT MINI TOURNAMENTS (Last 50) ===");
  const mSnapshot = await db.collection("mini_tournaments")
    .orderBy("created_at", "desc")
    .limit(50)
    .get();
  
  mSnapshot.forEach(doc => {
    const data = doc.data();
    console.log(`Mini Tournament: ${doc.id} | Title: ${data.title} | Organizer: ${data.organizer_id}`);
  });
}

recentScan().catch(console.error);
