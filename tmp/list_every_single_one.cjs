
const admin = require("firebase-admin");

const privateKey = "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n";

const serviceAccount = {
  projectId: "revallo-gaming-7642a",
  clientEmail: "firebase-adminsdk-fbsvc@revallo-gaming-7642a.iam.gserviceaccount.com",
  privateKey: privateKey.replace(/\\n/g, "\n")
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  });
}

const db = admin.firestore();

async function listEverySingleOne() {
  console.log("=== LISTING EVERY SINGLE REGULAR TOURNAMENT ===");
  const tSnapshot = await db.collection("tournaments").get();
  tSnapshot.forEach(doc => {
    console.log(`Tournament ID: ${doc.id} | Data: ${JSON.stringify(doc.data())}`);
  });

  console.log("\n=== LISTING EVERY SINGLE MINI TOURNAMENT ===");
  const mSnapshot = await db.collection("mini_tournaments").get();
  mSnapshot.forEach(doc => {
    console.log(`Mini Tournament ID: ${doc.id} | Data: ${JSON.stringify(doc.data())}`);
  });
}

listEverySingleOne().catch(console.error);
