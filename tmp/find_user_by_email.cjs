
const admin = require("firebase-admin");

const privateKey = "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n";

const serviceAccount = {
  projectId: "revallo-gaming-7642a",
  clientEmail: "firebase-adminsdk-fbsvc@revallo-gaming-7642a.iam.gserviceaccount.com",
  privateKey: privateKey.replace(/\\n/g, "\n")
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  });
}

const auth = admin.auth();
const db = admin.firestore();

async function findUserByEmail() {
  const email = "gameplaysadryan@gmail.com";
  try {
    const userRecord = await auth.getUserByEmail(email);
    console.log(`Successfully fetched user data for email: ${email}`);
    console.log(`UID: ${userRecord.uid}`);
    console.log(`DisplayName: ${userRecord.displayName}`);

    // Now check the profile in Firestore
    const profileDoc = await db.collection("profiles").doc(userRecord.uid).get();
    if (profileDoc.exists) {
        console.log(`Profile found in Firestore for UID: ${userRecord.uid}`);
        console.log(`Profile Data: ${JSON.stringify(profileDoc.data())}`);
    } else {
        console.log(`No profile found in Firestore for UID: ${userRecord.uid}`);
    }

    // List tournaments for this UID
    const tSnapshot = await db.collection("tournaments").where("organizer_id", "==", userRecord.uid).get();
    console.log(`Tournaments found for UID ${userRecord.uid}: ${tSnapshot.size}`);
    tSnapshot.forEach(doc => console.log(` - Tournament: ${doc.id} | Title: ${doc.data().title}`));

    const mSnapshot = await db.collection("mini_tournaments").where("organizer_id", "==", userRecord.uid).get();
    console.log(`Mini Tournaments found for UID ${userRecord.uid}: ${mSnapshot.size}`);
    mSnapshot.forEach(doc => console.log(` - Mini Tournament: ${doc.id} | Title: ${doc.data().title}`));

  } catch (error) {
    console.error("Error fetching user data:", error);
  }
}

findUserByEmail();
