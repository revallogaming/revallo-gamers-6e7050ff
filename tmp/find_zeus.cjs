
const admin = require("firebase-admin");

const privateKey = "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n";

const serviceAccount = {
  projectId: "revallo-gaming-7642a",
  clientEmail: "firebase-adminsdk-fbsvc@revallo-gaming-7642a.iam.gserviceaccount.com",
  privateKey: privateKey.replace(/\\n/g, "\n")
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  });
}

const db = admin.firestore();

async function findUserAndTournaments() {
  const nickname = "Zeus_Apollo";
  console.log(`Searching for profile with nickname: ${nickname}`);
  
  const pSnapshot = await db.collection("profiles").where("nickname", "==", nickname).get();
  
  if (pSnapshot.empty) {
    console.log("No profile found with that nickname.");
    return;
  }

  const userDoc = pSnapshot.docs[0];
  const userId = userDoc.id;
  console.log(`Found Profile ID: ${userId}`);

  console.log("\nSearching for tournaments associated with this ID...");
  
  const tSnapshot = await db.collection("tournaments").where("organizer_id", "==", userId).get();
  console.log(`Found ${tSnapshot.size} regular tournaments with organizer_id: ${userId}`);
  tSnapshot.forEach(doc => console.log(` - ${doc.id}: ${doc.data().title}`));

  const tSnapshotLegacy = await db.collection("tournaments").where("creatorId", "==", userId).get();
  console.log(`Found ${tSnapshotLegacy.size} regular tournaments with legacy creatorId: ${userId}`);
  tSnapshotLegacy.forEach(doc => console.log(` - [LEGACY] ${doc.id}: ${doc.data().title}`));

  const mSnapshot = await db.collection("mini_tournaments").where("organizer_id", "==", userId).get();
  console.log(`Found ${mSnapshot.size} mini tournaments with organizer_id: ${userId}`);
  mSnapshot.forEach(doc => console.log(` - ${doc.id}: ${doc.data().title}`));
}

findUserAndTournaments().catch(console.error);
