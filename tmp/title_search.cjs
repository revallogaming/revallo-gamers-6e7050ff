
const admin = require("firebase-admin");

const privateKey = "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n";

const serviceAccount = {
  projectId: "revallo-gaming-7642a",
  clientEmail: "firebase-adminsdk-fbsvc@revallo-gaming-7642a.iam.gserviceaccount.com",
  privateKey: privateKey.replace(/\\n/g, "\n")
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  });
}

const db = admin.firestore();

async function titleSearch() {
  const terms = ["Zeus", "Apollo"];
  console.log(`Searching for tournaments with terms in title: ${terms.join(", ")}`);

  async function searchCol(col) {
    const snap = await db.collection(col).get();
    snap.forEach(doc => {
      const title = doc.data().title || "";
      let match = false;
      for(const t of terms) {
        if(title.toLowerCase().includes(t.toLowerCase())) {
          match = true;
          break;
        }
      }
      if(match) {
        console.log(`MATCH in ${col}: ${doc.id} | Title: ${title} | organizer_id: ${doc.data().organizer_id}`);
      }
    });
  }

  await searchCol("tournaments");
  await searchCol("mini_tournaments");
}

titleSearch().catch(console.error);
