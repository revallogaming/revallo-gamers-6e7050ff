
const admin = require("firebase-admin");

const privateKey = "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n";

const serviceAccount = {
  projectId: "revallo-gaming-7642a",
  clientEmail: "firebase-adminsdk-fbsvc@revallo-gaming-7642a.iam.gserviceaccount.com",
  privateKey: privateKey.replace(/\\n/g, "\n")
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  });
}

const db = admin.firestore();

async function verifyMigration() {
  const longUID = "tjYoxEwnOAf1WmCJpWG0rqj4dqJ3";
  
  console.log("=== FINAL VERIFICATION ===");
  
  const pDoc = await db.collection("profiles").doc(longUID).get();
  console.log(`Long UID Profile: ${pDoc.exists ? 'EXISTS' : 'MISSING'}`);
  if (pDoc.exists) console.log(`- Nickname: ${pDoc.data().nickname}`);

  const cDoc = await db.collection("user_credits").doc(longUID).get();
  console.log(`Long UID Credits: ${cDoc.exists ? 'EXISTS' : 'MISSING'}`);
  if (cDoc.exists) console.log(`- Credits: ${cDoc.data().credits}`);

  const tSnap = await db.collection("tournaments").where("organizer_id", "==", longUID).get();
  console.log(`Tournaments linked to Long UID: ${tSnap.size}`);
  tSnap.forEach(doc => console.log(`- ${doc.id}: ${doc.data().title}`));

  const mSnap = await db.collection("mini_tournaments").where("organizer_id", "==", longUID).get();
  console.log(`Mini Tournaments linked to Long UID: ${mSnap.size}`);
  mSnap.forEach(doc => console.log(`- ${doc.id}: ${doc.data().title}`));
}

verifyMigration().catch(console.error);
