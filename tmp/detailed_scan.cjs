
const admin = require("firebase-admin");

const privateKey = "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n";

const serviceAccount = {
  projectId: "revallo-gaming-7642a",
  clientEmail: "firebase-adminsdk-fbsvc@revallo-gaming-7642a.iam.gserviceaccount.com",
  privateKey: privateKey.replace(/\\n/g, "\n")
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  });
}

const db = admin.firestore();

async function detailedScan() {
  const nicknames = ["Zeus_Apollo", "Zeus_Apollo3"];
  console.log(`Starting detailed scan for nicknames: ${nicknames.join(", ")}`);

  async function scanCol(colName) {
    console.log(`\n--- SCANNING ${colName.toUpperCase()} ---`);
    const snap = await db.collection(colName).get();
    snap.forEach(doc => {
      const data = doc.data();
      const dataStr = JSON.stringify(data).toLowerCase();
      let found = false;
      for (const nick of nicknames) {
        if (dataStr.includes(nick.toLowerCase())) {
          found = true;
          break;
        }
      }
      if (found) {
        console.log(`MATCH found in ${colName} ID ${doc.id}: ${JSON.stringify(data)}`);
      }
    });
  }

  await scanCol("tournaments");
  await scanCol("mini_tournaments");
  await scanCol("profiles");
}

detailedScan().catch(console.error);
