...
async function globalSearch() {
  const idsToSearch = ["VgETyxpXqNeBYabcg0KB", "tjYoxEwnOAf1WmCJpWG0"];
  const collections = ["tournaments", "mini_tournaments"];
  
  for (const col of collections) {
    console.log(`\n--- GLOBAL SEARCH IN ${col.toUpperCase()} ---`);
    const snap = await db.collection(col).get();
    snap.forEach(doc => {
      const dataStr = JSON.stringify(doc.data());
      for (const id of idsToSearch) {
        if (dataStr.includes(id)) {
            console.log(`FOUND ${id} in ${col} ID ${doc.id}: ${doc.data().title}`);
            break;
        }
      }
    });
