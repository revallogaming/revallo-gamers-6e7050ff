
const admin = require("firebase-admin");

const privateKey = "-----BEGIN PRIVATE KEY-----\\\n-----END PRIVATE KEY-----\n";

const serviceAccount = {
  projectId: "revallo-gaming-7642a",
  clientEmail: "firebase-adminsdk-fbsvc@revallo-gaming-7642a.iam.gserviceaccount.com",
  privateKey: privateKey.replace(/\\n/g, "\n")
};

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert(serviceAccount),
  });
}

const db = admin.firestore();

async function extremeScan() {
  const searchTerms = ["Zeus", "Apollo", "gameplaysadryan", "tjYoxEwnOAf1WmCJpWG0", "VgETyxpXqNeBYabcg0KB"];
  console.log(`Extreme scanning all documents for terms: ${searchTerms.join(", ")}`);

  async function scanCollection(collectionName) {
    console.log(`--- SCANNING ${collectionName.toUpperCase()} ---`);
    const snapshot = await db.collection(collectionName).get();
    snapshot.forEach(doc => {
        const data = doc.data();
        const dataStr = JSON.stringify(data);
        let found = false;
        for (const term of searchTerms) {
            if (dataStr.toLowerCase().includes(term.toLowerCase())) {
                found = true;
                break;
            }
        }
        if (found) {
            console.log(`MATCH in ${collectionName}: ${doc.id} | Data: ${dataStr}`);
        }
    });
  }

  await scanCollection("tournaments");
  await scanCollection("mini_tournaments");
  await scanCollection("profiles");
}

extremeScan().catch(console.error);
